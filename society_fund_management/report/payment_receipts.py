...
class PaymentReceipts(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'getHeader': self._period_get,
        })
        self.context = context

    def _period_get(self, form):
        return form['header']
    ...
    def _payment_lines_get(self, form):
        ...
        payment_ids = account_payment_obj.search(self.cr, self.uid,[('partner_id', '=', form['partner_id'][0]), ('payment_date', '>=', form['start_date']), ('payment_date', '<=', form['end_date']), ('journal_id.type', 'in', ('bank', 'cash'))])
        payments = account_payment_obj.browse(self.cr, self.uid, payment_ids)
        ...
        months = [i for i in range(1,r.months+1)]
        ...
        for period in months:
            for voucher in payments:
                date = datetime.strptime(voucher.payment_date, '%Y-%m-%d')
                if date.month == period:
                    for r in res:
                         if r.has_key('period_id') and r['period_id'] == period:
                            r.update({
                                'partner_name': voucher.partner_id.name + ' ' +  '(' + voucher.partner_id.account_no + ')',
                                'date':datetime.strptime(voucher.payment_date, '%Y-%m-%d').strftime('%d/%m/%Y'),
                                ...
                                'loan_remaining': (voucher.loan_id.loan_amount - ((voucher.count_emi_paid if voucher.count_emi_paid > 0 else 1) * voucher.loan_emi)),
